...
for (let index = 0; index < iterations; index += 1) {
  ...
  const result = spawnSync(command, args, { cwd: root, input: `${JSON.stringify({ op: 'inspect_engine' })}\n`, encoding: 'utf8', timeout: 120000 });
...
const report = { schemaVersion: 2, engine: 'acorde@1.2.0', fixturePath: path.relative(root, inputPath), iterations, engine_identity: identity, environment: { platform: process.platform, arch: process.arch, os_release: os.release(), node: process.version }, cold_start_ms: summary, metric: 'spawn-to-engine-ready' };
