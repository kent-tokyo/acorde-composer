...
const REQUIRED_METRICS = new Set(['parse', 'load', 'render']);
...
function validatePerformanceBenchmark(inputPath = path.resolve('qa/performance-benchmark.json'), rootDir = process.cwd()) {
  ...
  for (const profile of Array.isArray(input?.profiles) ? input.profiles : []) {
    ...
    if (!Array.isArray(profile.metrics) || profile.metrics.length !== REQUIRED_METRICS.size || !profile.metrics.every((metric) => REQUIRED_METRICS.has(metric))) errors.push(`metrics must include parse/load/render: ${profile.id}`);
