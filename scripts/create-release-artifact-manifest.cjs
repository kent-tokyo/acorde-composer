...
function packagedArtifactFiles(distDir) {
  if (!fs.existsSync(distDir)) return [];
  return fs.readdirSync(distDir, { withFileTypes: true })
    .filter((entry) => entry.isFile() && /\.(dmg|zip|exe|appimage|deb|rpm)$/i.test(entry.name))
...
function createReleaseArtifactManifest({ distDir = path.resolve('dist'), version, commit = null } = {}) {
  ...
  const distributableFiles = packagedArtifactFiles(distDir);
  ...
  const noticeIncluded = artifactRoots.some((root) => containsNotice(root));
  const artifacts = distributableFiles.length
    ? distributableFiles.map((file) => ({ name: path.basename(file), sha256: sha256File(file), sbom: true, notice: noticeIncluded, provenance: true }))
    : artifactRoots.map((root) => ({ name: path.basename(root), sha256: directoryDigest(root), sbom: true, notice: containsNotice(root), provenance: true }));
...
module.exports = { createReleaseArtifactManifest, directoryDigest, filesUnder, packagedArtifactFiles };
