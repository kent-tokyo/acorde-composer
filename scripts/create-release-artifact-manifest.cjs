...
function createReleaseArtifactManifest({ distDir = path.resolve('dist'), version, commit = null } = {}) {
  ...
  const artifactRoots = fs.existsSync(distDir) ? fs.readdirSync(distDir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && entry.name.endsWith('-unpacked')).map((entry) => path.join(distDir, entry.name)).sort() : [];
  ...
  const artifacts = artifactRoots.map((root) => ({ name: path.basename(root), sha256: directoryDigest(root), sbom: true, notice: fs.existsSync(path.join(root, 'resources', 'app', 'NOTICE.md')), provenance: true }));
