...
(async () => {
  ...
  const report = { schemaVersion: 1, engine: 'acorde@1.1.7', fixturePath: path.relative(root, inputPath), iterations, eventCount, parse_ms: summarize(parse), playback_ms: summarize(playback), harness_memory: { rss_before_bytes: rssBefore, rss_after_bytes: rssAfter, rss_delta_bytes: rssAfter - rssBefore } };
