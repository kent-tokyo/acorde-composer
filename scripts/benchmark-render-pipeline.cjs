...
(async () => {
  ...
  const report = { schemaVersion: 1, engine: 'acorde@1.2.0', fixturePath: path.relative(root, inputPath), iterations, layout_ms: summarize(metrics.layout_ms), svg_ms: summarize(metrics.svg_ms), svg_metadata_ms: summarize(metrics.svg_metadata_ms), repeat_render_ms: summarize(metrics.repeat_render_ms) };
