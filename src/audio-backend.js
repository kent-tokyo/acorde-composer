(function () {
  class OscillatorAudioBackend {
    ...
    schedule(events, start, channel = {}, channelKey = null) {
      ...
      events.forEach((item) => {
        if (item.decoded_sample) { this.scheduleDecodedSample(item.decoded_sample, item, start, channelGain); return; }
    ...
    scheduleDecodedSample(sample, event, start, destination = this.master) {
      ...
      source.connect(gain).connect(destination); source.start(when); source.stop(when + duration);
      this.nodes.add(source); source.addEventListener('ended', () => this.nodes.delete(source));
      return source;
    }
