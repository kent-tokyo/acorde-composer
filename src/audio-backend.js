(function () {
  class OscillatorAudioBackend {
    ...
    async resume() { this.context ||= new AudioContext(); this.master ||= this.context.createGain(); this.master.connect(this.context.destination); await this.context.resume(); return this.context; }
    ...
    async dispose() { this.stopAll(); if (this.context && this.context.state !== 'closed') await this.context.close(); this.context = null; this.master = null; }
  }
