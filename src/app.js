...
let scheduledEvents = [];
let playbackEventCursor = 0;
const playbackNoteByAddress = new Map();
...
window.acorde.playbackPosition = async (...args) => {
  if (playbackPositionInFlight) return null;
  playbackPositionInFlight = true;
  try { return await basePlaybackPosition(...args); } finally { playbackPositionInFlight = false; }
};
function ensureUiDialog() { let dialog = $('ui-dialog'); if (dialog) return dialog; dialog = document.createElement('dialog'); dialog.id = 'ui-dialog'; dialog.className = 'ui-dialog'; document.body.append(dialog); return dialog; }
  ...
  list.replaceChildren(...items.map((item) => {
    ...
    for (const action of ['accept', 'reject', 'correct']) {
      ...
      button.addEventListener('click', async () => {
        const value = action === 'correct' ? await uiPrompt('Correction value', item.proposal?.value || '') : null;
        const correction = action === 'correct' && value !== null ? { type: 'correction', value: value.trim() } : null;
...
function updateRenderedScore(svg) {
  ...
  $('score').querySelectorAll('[data-acorde-kind="note"]').forEach((note) => {
    ...
    note.addEventListener('click', async (event) => {
      ...
      if (toolMode === 'tie') {
        try { await applyCommand({ type: 'toggle_tie', part_index: address.part, staff_index: address.staff, measure_index: address.measure, voice: address.voice, note_index: address.note }, 'ToggleTie'); } catch (error) { uiAlert(`タイを変更できませんでした: ${userFacingError(error)}`); }
        return;
      }
      if (toolMode === 'slur') {
        if (!slurStart) { slurStart = address; selectRenderedNote(note); return; }
        try { await applyCommand({ type: 'toggle_slur', start: slurStart, end: address }, 'ToggleSlur'); slurStart = null; } catch (error) { uiAlert(`スラーを変更できませんでした: ${userFacingError(error)}`); }
        return;
      }
      if (toolMode === 'hairpin') {
        ...
        try { await applyCommand({ type: 'add_hairpin', part_index: address.part, staff_index: address.staff, measure_index: address.measure, voice: address.voice, start_note_idx: hairpinStart.note, end_note_idx: address.note, kind: $('hairpin-kind').value }, 'AddHairpin'); hairpinStart = null; } catch (error) { uiAlert(`ヘアピンを変更できませんでした: ${userFacingError(error)}`); }
        return;
      }
      if (toolMode === 'ottava') {
      ...
      if (toolMode === 'pedal') {
        ...
        try { await applyCommand({ type: 'add_pedal', part_index: address.part, staff_index: address.staff, measure_index: address.measure, voice: address.voice, start_note_idx: pedalStart.note, end_note_idx: address.note }, 'AddPedal'); pedalStart = null; } catch (error) { uiAlert(`ペダルを変更できませんでした: ${userFacingError(error)}`); }
        return;
      }
...
function addComposerAbcExportWarnings(report, score) {
  ...
  const measures = (score?.parts || []).flatMap((part) => (part.staves || []).flatMap((staff) => staff.measures || []));
  const notes = measures.flatMap((measure) => measure.voices || []).flat();
  const hasUnrepresentableVoices = (score?.parts || []).some((part) => (part.staves || []).length > 1 || (part.staves || []).some((staff) => (staff.measures || []).some((measure) => (measure.voices || []).length > 1)));
  const hasUnrepresentableAnnotations = notes.some((note) => note.articulations?.length || note.dynamic || note.tie_start || note.tie_end || note.tuplet || note.chord_symbol || note.is_grace || note.ottava_start || note.ottava_end || note.lyric || note.pedal_start || note.pedal_end || note.slur_start || note.slur_end || note.arpeggiate || note.technique_text || note.fingering != null || note.string_number != null || (note.note_head && note.note_head !== 'Normal') || note.is_cue || note.trill_line_start || note.trill_line_end);
  const hasUnrepresentableSpanners = notes.some((note) => note.glissando_start || note.glissando_end || note.cross_staff);
  const hasStyledText = measures.some((measure) => measure.texts?.length);
  const hasUnrepresentableMeasureData = measures.some((measure) => measure.navigation || measure.volta || measure.expression_text || measure.rehearsal_mark || measure.tempo || measure.time_sig || measure.key_sig || (measure.barline_left && measure.barline_left !== 'Normal') || (measure.barline_right && measure.barline_right !== 'Normal')) || ['lyricist', 'copyright', 'work_number', 'movement_title'].some((field) => score?.metadata?.[field]);
  if (hasUnrepresentableVoices) next.diagnostics.push({ code: 'composer.abc-lossy-voices', severity: 'Warning', loss_reason: 'ABC export preserves only the first staff and voice of each part' });
  if (hasUnrepresentableAnnotations || hasUnrepresentableMeasureData) next.diagnostics.push({ code: 'composer.abc-lossy-notation', severity: 'Warning', loss_reason: 'ABC export preserves basic pitch, duration, rest, and chord tones; other notation annotations may be omitted' });
  if (hasUnrepresentableSpanners) next.diagnostics.push({ code: 'composer.abc-lossy-spanners', severity: 'Warning', loss_reason: 'ABC export cannot preserve glissando or cross-staff placement' });
  if (hasStyledText) next.diagnostics.push({ code: 'composer.abc-lossy-text-styles', severity: 'Warning', loss_reason: 'ABC export cannot preserve MusicXML text styles and measure-level styled text' });
  return next;
}
...
function syncEditorAfterScoreChange() {
  ...
  if (currentScore) saveMixer();
  const selector = $('part-select');
  if (selector && currentScore) {
    const previous = activePartIndex;
    selector.replaceChildren(...currentScore.parts.map((part, index) => { const option = document.createElement('option'); option.value = index; option.textContent = part.name || `Part ${index + 1}`; return option; }));
    activePartIndex = Math.min(previous, Math.max(0, currentScore.parts.length - 1));
    selector.value = String(activePartIndex);
  }
...
$('chord-button').addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const item = currentScore.parts?.[part]?.staves?.[staff]?.measures?.[measure]?.voices?.[voice]?.[note]; const value = await uiPrompt('Chord symbol (e.g. C, Am7, F#)', item?.chord_symbol?.root || ''); if (value === null) return; const match = value.trim().match(/^([A-Ga-g])([#b]?)(.*)$/); if (value.trim() && !match) return uiAlert('コード記号は C / Am7 / F# の形式で入力してください。'); const root = match ? `${match[1].toUpperCase()}${match[2]}` : ''; const suffix = match ? match[3].toLowerCase() : ''; const kind = suffix === 'm' ? 'minor' : suffix === 'maj7' ? 'major-seventh' : suffix === 'm7' ? 'minor-seventh' : suffix === '7' ? 'dominant' : suffix === 'dim' ? 'diminished' : suffix === 'sus2' ? 'suspended-second' : suffix === 'sus4' ? 'suspended-fourth' : suffix === '6' ? 'major-sixth' : 'major'; try { await applyCommand({ type: 'set_chord_symbol', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, chord: root ? { root, kind, bass: null } : null }, 'SetChordSymbol'); } catch (error) { uiAlert(`コード記号を変更できませんでした: ${userFacingError(error)}`); } });
$('rehearsal-button').addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const measure = selectedAddress.measure; const current = currentScore.parts?.[selectedAddress.part]?.staves?.[selectedAddress.staff]?.measures?.[measure]?.rehearsal || ''; const text = await uiPrompt(`Rehearsal mark for measure ${measure + 1}`, current); if (text === null) return; try { await applyCommand({ type: 'set_rehearsal_mark', measure_index: measure, text: text.trim() || null }, 'SetRehearsalMark'); } catch (error) { uiAlert(`リハーサルマークを変更できませんでした: ${userFacingError(error)}`); } });
$('measure-tempo-button').addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const measure = selectedAddress.measure; const current = currentScore.parts?.[selectedAddress.part]?.staves?.[selectedAddress.staff]?.measures?.[measure]?.tempo || ''; const value = await uiPrompt(`Tempo BPM for measure ${measure + 1} (blank clears override)`, current ? String(current) : ''); if (value === null) return; const bpm = value.trim() ? Number(value) : null; if (bpm !== null && (!Number.isInteger(bpm) || bpm < 1 || bpm > 400)) return uiAlert('テンポは1〜400の整数で入力してください。'); try { await applyCommand({ type: 'set_tempo_at_measure', measure_index: measure, bpm }, 'SetTempoAtMeasure'); } catch (error) { uiAlert(`小節テンポを変更できませんでした: ${userFacingError(error)}`); } });
$('accidental-select').addEventListener('change', async () => { if (!currentScore || !selectedAddress || $('accidental-select').value === '') return; const { part, staff, measure, voice, note } = selectedAddress; const item = currentScore.parts?.[part]?.staves?.[staff]?.measures?.[measure]?.voices?.[voice]?.[note]; const pitch = item?.pitches?.[0]; if (!pitch || item.is_rest) { $('accidental-select').value = ''; return; } try { await applyCommand({ type: 'add_pitch', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, pitch: { ...pitch, alter: Number($('accidental-select').value) } }, 'SetAccidental'); } catch (error) { uiAlert(`臨時記号を変更できませんでした: ${userFacingError(error)}`); } finally { $('accidental-select').value = ''; } });
const extendedArticulations = [['Mordent', 'Mordent'], ['InvertedMordent', 'Inverted mordent'], ['Turn', 'Turn'], ['InvertedTurn', 'Inverted turn'], ['Shake', 'Shake'], ['{"Tremolo":1}', 'Tremolo'], ['BreathMark', 'Breath mark'], ['Caesura', 'Caesura']]; extendedArticulations.forEach(([value, label]) => { if ([...$('articulation-select').options].some((option) => option.value === value)) return; $('articulation-select').append(new Option(label, value)); });
$('articulation-select').addEventListener('change', async () => { if (!currentScore || !selectedAddress || !$('articulation-select').value) return; const { part, staff, measure, voice, note } = selectedAddress; let articulation = $('articulation-select').value; try { articulation = JSON.parse(articulation); } catch {} try { await applyCommand({ type: 'toggle_articulation', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, articulation }, 'ToggleArticulation'); } catch (error) { uiAlert(`アーティキュレーションを変更できませんでした: ${userFacingError(error)}`); } finally { $('articulation-select').value = ''; } });
let highlightedPlaybackNote = null;
let highlightedPlaybackAddress = null;
...
async function applyScoreSettingsFromButton(event) {
  ...
  accessibilityState.highContrast = $('high-contrast').checked;
  saveAccessibilityPreferences();
  if (!currentScore) return;
  const bpm = Number($('tempo-input').value);
  const midiChannel = Number($('midi-channel-input').value) - 1;
  const midiProgram = Number($('midi-program-input').value) - 1;
  const semitones = Number($('transpose-input').value);
  const staffIndex = Number($('staff-select')?.value || 0);
  if (!Number.isInteger(bpm) || bpm < 1 || bpm > 400 || !Number.isInteger(midiChannel) || midiChannel < 0 || midiChannel > 15 || !Number.isInteger(midiProgram) || midiProgram < 0 || midiProgram > 127 || !Number.isInteger(semitones) || semitones < -48 || semitones > 48) return;
  try {
    await applyCommand({ type: 'batch', commands: [{ type: 'set_metadata', title: $('title-input').value.trim() || null, composer: $('composer-input').value.trim() || null, lyricist: $('lyricist-input').value.trim() || null, copyright: $('copyright-input').value.trim() || null, work_number: $('work-number-input').value.trim() || null, movement_title: $('movement-title-input').value.trim() || null }, { type: 'set_tempo', bpm }, { type: 'set_midi_instrument', part_index: activePartIndex, midi_channel: midiChannel, midi_program: midiProgram }, { type: 'set_clef', part_index: activePartIndex, staff_index: staffIndex, clef: $('clef-select').value }, { type: 'set_transpose', part_index: activePartIndex, staff_index: staffIndex, semitones }], label: 'UpdateScoreSettings' }, 'UpdateScoreSettings');
    $('score-settings').close();
  } catch (error) { uiAlert(`スコア設定を変更できませんでした: ${userFacingError(error)}`); }
}
...
function groupAdvancedNotationControls() { const first = $('tuplet-select'); const parent = first?.parentElement; if (!first || !parent || parent.querySelector('.advanced-tools')) return; const wrapper = document.createElement('details'); wrapper.className = 'advanced-tools'; wrapper.setAttribute('aria-label', 'Advanced notation'); const summary = document.createElement('summary'); summary.textContent = 'Advanced'; wrapper.append(summary); parent.insertBefore(wrapper, first); ['tuplet-select', 'dynamic-select', 'grace-select', 'accidental-select', 'articulation-select'].forEach((id) => wrapper.appendChild($(id))); }
groupAdvancedNotationControls();
function installRepeatControls() { const start = document.createElement('button'); start.id = 'repeat-start-button'; start.className = 'quiet'; start.textContent = 'Repeat Start'; const end = document.createElement('button'); end.id = 'repeat-end-button'; end.className = 'quiet'; end.textContent = 'Repeat End'; $('score-meta').insertBefore(start, $('mixer-button')); $('score-meta').insertBefore(end, $('mixer-button')); const apply = async (side, barline) => { if (!currentScore || !selectedAddress) return; const measure = currentScore.parts?.[selectedAddress.part]?.staves?.[selectedAddress.staff]?.measures?.[selectedAddress.measure]; const current = side === 'left' ? measure?.barline_left : measure?.barline_right; const next = current === barline ? 'Normal' : barline; try { await applyCommand({ type: 'set_barline', measure_index: selectedAddress.measure, side, barline: next }, next === 'Normal' ? 'ClearBarline' : 'SetBarline'); } catch (error) { uiAlert(`反復記号を変更できませんでした: ${userFacingError(error)}`); } }; start.addEventListener('click', () => apply('left', 'RepeatStart')); end.addEventListener('click', () => apply('right', 'RepeatEnd')); }
installRepeatControls();
function installNavigationControls() { const navigation = document.createElement('button'); navigation.id = 'navigation-button'; navigation.className = 'quiet'; navigation.textContent = 'Navigation'; const volta = document.createElement('button'); volta.id = 'volta-button'; volta.className = 'quiet'; volta.textContent = 'Volta'; $('score-meta').insertBefore(navigation, $('mixer-button')); $('score-meta').insertBefore(volta, $('mixer-button')); navigation.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const measure = selectedAddress.measure; const current = currentScore.parts?.[selectedAddress.part]?.staves?.[selectedAddress.staff]?.measures?.[measure]?.navigation || ''; const value = await uiPrompt('Navigation mark (Segno, Coda, Fine, DaCapo...)', current); if (value === null) return; const allowed = ['Segno', 'Coda', 'Fine', 'DaCapo', 'DaCapoAlFine', 'DaCapoAlCoda', 'DalSegno', 'DalSegnoAlFine', 'DalSegnoAlCoda', 'ToCoda']; if (value.trim() && !allowed.includes(value.trim())) return uiAlert(`Navigation markは ${allowed.join(', ')} のいずれかです。`); try { await applyCommand({ type: 'set_navigation_mark', measure_index: measure, mark: value.trim() || null }, 'SetNavigationMark'); } catch (error) { uiAlert(`ナビゲーション記号を変更できませんでした: ${userFacingError(error)}`); } }); volta.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const measure = selectedAddress.measure; const current = currentScore.parts?.[selectedAddress.part]?.staves?.[selectedAddress.staff]?.measures?.[measure]?.volta; const value = await uiPrompt('Volta ending number (blank clears, e.g. 1 or 2)', current?.number ? String(current.number) : ''); if (value === null) return; const number = value.trim() ? Number(value) : null; if (number !== null && (!Number.isInteger(number) || number < 1 || number > 8)) return uiAlert('Volta numberは1〜8の整数で入力してください。'); try { await applyCommand({ type: 'set_volta', measure_index: measure, volta: number === null ? null : { number, kind: 'begin_end' } }, 'SetVolta'); } catch (error) { uiAlert(`Voltaを変更できませんでした: ${userFacingError(error)}`); } }); }
...
function installGuitarControls() { const stringButton = document.createElement('button'); stringButton.id = 'string-number-button'; stringButton.className = 'quiet'; stringButton.textContent = 'String'; const techniqueButton = document.createElement('button'); techniqueButton.id = 'technique-button'; techniqueButton.className = 'quiet'; techniqueButton.textContent = 'Technique'; $('score-meta').insertBefore(stringButton, $('mixer-button')); $('score-meta').insertBefore(techniqueButton, $('mixer-button')); stringButton.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const item = currentScore.parts?.[part]?.staves?.[staff]?.measures?.[measure]?.voices?.[voice]?.[note]; const value = await uiPrompt('String number: blank clears, 1-12', item?.string_number == null ? '' : String(item.string_number)); if (value === null) return; const stringNumber = value.trim() ? Number(value) : null; if (stringNumber !== null && (!Number.isInteger(stringNumber) || stringNumber < 1 || stringNumber > 12)) return uiAlert('String numberは1〜12の整数です。'); try { await applyCommand({ type: 'set_string_number', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, string_number: stringNumber }, 'SetStringNumber'); } catch (error) { uiAlert(`弦番号を変更できませんでした: ${userFacingError(error)}`); } }); techniqueButton.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const value = await uiPrompt('Technique: Bend, Slide, HammerOn, PullOff (blank clears)', ''); if (value === null) return; const normalized = value.trim().toLowerCase(); const technique = normalized ? ({ bend: 'Bend', slide: 'Slide', hammeron: 'HammerOn', 'hammer-on': 'HammerOn', pulloff: 'PullOff', 'pull-off': 'PullOff' }[normalized] || null) : null; if (value.trim() && !technique) return uiAlert('Techniqueは Bend / Slide / HammerOn / PullOff のいずれかです。'); try { await applyCommand({ type: 'set_guitar_technique', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, technique }, 'SetGuitarTechnique'); } catch (error) { uiAlert(`奏法を変更できませんでした: ${userFacingError(error)}`); } }); }
installGuitarControls();
function installOrnamentControls() { const arpeggio = document.createElement('button'); arpeggio.id = 'arpeggio-button'; arpeggio.className = 'quiet'; arpeggio.textContent = 'Arpeggio'; const trill = document.createElement('button'); trill.id = 'trill-button'; trill.className = 'quiet'; trill.textContent = 'Trill line'; $('score-meta').insertBefore(arpeggio, $('mixer-button')); $('score-meta').insertBefore(trill, $('mixer-button')); arpeggio.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const value = await uiPrompt('Arpeggio: up, down, or blank to clear', 'up'); if (value === null) return; const direction = value.trim() ? value.trim().toLowerCase() === 'up' ? true : value.trim().toLowerCase() === 'down' ? false : undefined : null; if (direction === undefined) return uiAlert('Arpeggioは up / down のいずれかです。'); try { await applyCommand({ type: 'set_arpeggio', part_index: part, staff_index: staff, measure_index: measure, voice_index: voice, note_index: note, direction }, 'SetArpeggio'); } catch (error) { uiAlert(`アルペジオを変更できませんでした: ${userFacingError(error)}`); } }); let start = null; trill.addEventListener('click', () => { start = selectedAddress; if (start) uiAlert('Trill line: 終了音符をクリックしてください。'); }); window.addEventListener('score-note-click', async (event) => { if (!start || !event.detail || !currentScore) return; const end = event.detail; try { await applyCommand({ type: 'toggle_trill_line', start, end }, 'ToggleTrillLine'); } catch (error) { uiAlert(`トリルラインを変更できませんでした: ${userFacingError(error)}`); } finally { start = null; } }); }
installOrnamentControls();
function installAdvancedNotationControls() {
  const glissando = document.createElement('button'); glissando.id = 'glissando-button'; glissando.className = 'quiet'; glissando.textContent = 'Glissando';
  const crossStaff = document.createElement('button'); crossStaff.id = 'cross-staff-button'; crossStaff.className = 'quiet'; crossStaff.textContent = 'Cross-staff';
  $('score-meta').insertBefore(glissando, $('mixer-button')); $('score-meta').insertBefore(crossStaff, $('mixer-button'));
  let start = null;
  glissando.addEventListener('click', () => { if (!selectedAddress) return; start = selectedAddress; uiAlert('Glissando: 終了音符をクリックしてください。'); });
  window.addEventListener('score-note-click', async (event) => {
    if (!start || !event.detail || !currentScore) return;
    const end = event.detail;
    if (start.part !== end.part || start.staff !== end.staff || start.measure !== end.measure || start.voice !== end.voice || start.note >= end.note) { start = end; return; }
    try {
      await applyCommand({ type: 'batch', label: 'SetGlissando', commands: [
        { type: 'set_glissando', part_index: start.part, staff_index: start.staff, measure_index: start.measure, voice: start.voice, note_index: start.note, start: true, end: false },
        { type: 'set_glissando', part_index: end.part, staff_index: end.staff, measure_index: end.measure, voice: end.voice, note_index: end.note, start: false, end: true },
      ] }, 'SetGlissando');
    } catch (error) { uiAlert(`グリッサンドを変更できませんでした: ${userFacingError(error)}`); } finally { start = null; }
  });
  crossStaff.addEventListener('click', async () => {
    if (!currentScore || !selectedAddress) return;
    const staffCount = currentScore.parts?.[selectedAddress.part]?.staves?.length || 1;
    if (staffCount < 2) return uiAlert('Cross-staffには2つ以上の譜表が必要です。');
    const current = currentScore.parts[selectedAddress.part].staves[selectedAddress.staff].measures[selectedAddress.measure].voices[selectedAddress.voice][selectedAddress.note]?.cross_staff?.target_staff;
    const value = await uiPrompt(`Cross-staff target (1-${staffCount}, blank clears)`, current == null ? '' : String(current + 1));
    if (value === null) return;
    const target = value.trim() ? Number(value) - 1 : null;
    if (target !== null && (!Number.isInteger(target) || target < 0 || target >= staffCount || target === selectedAddress.staff)) return uiAlert('Cross-staff targetは別の譜表番号を指定してください。');
    try { await applyCommand({ type: 'set_cross_staff', part_index: selectedAddress.part, staff_index: selectedAddress.staff, measure_index: selectedAddress.measure, voice: selectedAddress.voice, note_index: selectedAddress.note, placement: target === null ? null : { target_staff: target, target_voice: null } }, 'SetCrossStaff'); } catch (error) { uiAlert(`Cross-staffを変更できませんでした: ${userFacingError(error)}`); }
  });
}
installAdvancedNotationControls();
function installTechniqueTextControl() { const button = document.createElement('button'); button.id = 'technique-text-button'; button.className = 'quiet'; button.textContent = 'Technique text'; $('score-meta').insertBefore(button, $('mixer-button')); button.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const item = currentScore.parts?.[part]?.staves?.[staff]?.measures?.[measure]?.voices?.[voice]?.[note]; const value = await uiPrompt('Technique text (pizz., arco, con sord.; blank clears)', item?.technique_text || ''); if (value === null) return; try { await applyCommand({ type: 'set_technique_text', part_index: part, staff_index: staff, measure_index: measure, voice, note_index: note, text: value.trim() || null }, 'SetTechniqueText'); } catch (error) { uiAlert(`奏法テキストを変更できませんでした: ${userFacingError(error)}`); } }); }
...
function installStemControl() { const button = document.createElement('button'); button.id = 'stem-button'; button.className = 'quiet'; button.textContent = 'Stem'; $('score-meta').insertBefore(button, $('mixer-button')); button.addEventListener('click', async () => { if (!currentScore || !selectedAddress) return; const { part, staff, measure, voice, note } = selectedAddress; const item = currentScore.parts?.[part]?.staves?.[staff]?.measures?.[measure]?.voices?.[voice]?.[note]; const value = await uiPrompt('Stem direction: auto, up, or down', item?.stem_up === true ? 'up' : item?.stem_up === false ? 'down' : 'auto'); if (value === null) return; const stemUp = value.trim().toLowerCase() === 'auto' ? null : value.trim().toLowerCase() === 'up' ? true : value.trim().toLowerCase() === 'down' ? false : undefined; if (stemUp === undefined) return uiAlert('Stem directionは auto / up / down のいずれかです。'); try { await applyCommand({ type: 'set_stem', part_index: part, staff_index: staff, measure_index: measure, voice_index: voice, note_index: note, stem_up: stemUp }, 'SetStem'); } catch (error) { uiAlert(`stem方向を変更できませんでした: ${userFacingError(error)}`); } }); }
installStemControl();
...
installPedalControl();
function installLayoutControls() { const multiRest = document.createElement('button'); multiRest.id = 'multi-rest-button'; multiRest.className = 'quiet'; multiRest.textContent = 'Multi-rest'; const pageBreak = document.createElement('button'); pageBreak.id = 'page-break-button'; pageBreak.className = 'quiet'; pageBreak.textContent = 'Page break'; const systemBreak = document.createElement('button'); systemBreak.id = 'system-break-button'; systemBreak.className = 'quiet'; systemBreak.textContent = 'System break'; $('score-meta').insertBefore(multiRest, $('mixer-button')); $('score-meta').insertBefore(pageBreak, $('mixer-button')); $('score-meta').insertBefore(systemBreak, $('mixer-button')); const measureAtSelection = () => selectedAddress?.measure; multiRest.addEventListener('click', async () => { const measure = measureAtSelection(); if (!currentScore || measure === undefined) return; const current = currentScore.parts?.[0]?.staves?.[0]?.measures?.[measure]?.multi_rest_count; const value = await uiPrompt('Multi-rest count (blank clears, 2-99)', current ? String(current) : ''); if (value === null) return; const count = value.trim() ? Number(value) : null; if (count !== null && (!Number.isInteger(count) || count < 2 || count > 99)) return uiAlert('Multi-rest countは2〜99の整数です。'); try { await applyCommand({ type: 'set_multi_rest', measure_index: measure, count }, 'SetMultiRest'); } catch (error) { uiAlert(`マルチレストを変更できませんでした: ${userFacingError(error)}`); } }); const toggleBreak = (type, button) => async () => { const measure = measureAtSelection(); if (!currentScore || measure === undefined) return; const current = currentScore.parts?.[0]?.staves?.[0]?.measures?.[measure]?.[type]; try { await applyCommand({ type: type === 'page_break' ? 'set_page_break' : 'set_system_break', measure_index: measure, value: !current }, type === 'page_break' ? 'SetPageBreak' : 'SetSystemBreak'); button.classList.toggle('active', !current); } catch (error) { uiAlert(`レイアウト区切りを変更できませんでした: ${userFacingError(error)}`); } }; pageBreak.addEventListener('click', toggleBreak('page_break', pageBreak)); systemBreak.addEventListener('click', toggleBreak('system_break', systemBreak)); }
installLayoutControls();
function installMidiInputControl() { const button = document.createElement('button'); button.id = 'midi-input-button'; button.className = 'quiet'; button.textContent = 'MIDI input'; const selector = document.createElement('select'); selector.id = 'midi-input-select'; selector.setAttribute('aria-label', 'MIDI input device'); const status = document.createElement('span'); status.id = 'midi-input-status'; status.className = 'muted'; status.textContent = 'MIDI off'; $('score-meta').insertBefore(selector, $('mixer-button')); $('score-meta').insertBefore(button, $('mixer-button')); $('score-meta').insertBefore(status, $('mixer-button')); selector.addEventListener('change', () => { mixerState.midiInputId = selector.value; saveMixer(); if (!midiAccess || !midiInput) return; const next = [...midiAccess.inputs.values()].find((input) => input.id === selector.value); if (!next) return; midiInput.onmidimessage = null; midiInput = next; midiInput.onmidimessage = handleMessage; button.textContent = 'Disconnect MIDI'; status.textContent = `MIDI: ${next.name || 'connected'}`; }); let midiAccess = null; const refreshInputs = () => { if (!midiAccess) return; const previous = selector.value; const inputs = [...midiAccess.inputs.values()]; selector.replaceChildren(...inputs.map((input) => { const option = document.createElement('option'); option.value = input.id; option.textContent = input.name || `MIDI input ${input.id}`; return option; })); const preferred = mixerState.midiInputId; if (inputs.some((input) => input.id === preferred)) selector.value = preferred; else if (inputs.some((input) => input.id === previous)) selector.value = previous; selector.disabled = inputs.length === 0; }; const handleMessage = async ({ data }) => { const [command, midi, velocity] = data || []; if ((command & 0xf0) !== 0x90 || !velocity || !currentScore) return; const part = currentScore.parts?.[activePartIndex]; const staffIndex = Number($('staff-select')?.value || 0); const staff = part?.staves?.[staffIndex]; const measureIndex = selectedAddress?.part === activePartIndex && selectedAddress?.staff === staffIndex ? selectedAddress.measure : 0; const voiceIndex = selectedAddress?.part === activePartIndex && selectedAddress?.staff === staffIndex ? selectedAddress.voice : activeVoiceIndex; const voice = staff?.measures?.[measureIndex]?.voices?.[voiceIndex]; if (!voice) return; const position = selectedAddress?.part === activePartIndex && selectedAddress?.staff === staffIndex && selectedAddress.measure === measureIndex && selectedAddress.voice === voiceIndex ? selectedAddress.note : voice.length; try { await applyCommand({ type: 'add_note', part_index: activePartIndex, staff_index: staffIndex, measure_index: measureIndex, voice: voiceIndex, position, pitch: pitchFromMidi(midi), duration: 'quarter', dot_count: 0, is_rest: false }, 'MidiInputNote'); } catch (error) { status.textContent = `MIDI error: ${userFacingError(error)}`; } }; const handleStateChange = ({ port }) => { refreshInputs(); if (port === midiInput && port.state === 'disconnected') { midiInput = null; button.textContent = 'MIDI input'; status.textContent = 'MIDI disconnected'; } }; button.addEventListener('click', async () => { if (midiInput) { midiInput.onmidimessage = null; midiInput = null; button.textContent = 'MIDI input'; status.textContent = 'MIDI off'; return; } if (!navigator.requestMIDIAccess) { status.textContent = 'Web MIDI unavailable'; return; } try { midiAccess = await navigator.requestMIDIAccess(); midiAccess.onstatechange = handleStateChange; refreshInputs(); const input = [...midiAccess.inputs.values()].find((candidate) => candidate.id === selector.value) || [...midiAccess.inputs.values()][0]; if (!input) { status.textContent = 'No MIDI input'; return; } selector.value = input.id; mixerState.midiInputId = input.id; saveMixer(); midiInput = input; midiInput.onmidimessage = handleMessage; button.textContent = 'Disconnect MIDI'; status.textContent = `MIDI: ${input.name || 'connected'}`; } catch (error) { status.textContent = `MIDI denied: ${userFacingError(error)}`; } }); }
installMidiInputControl();
function groupScoreMetaControls() {
  const meta = $('score-meta');
  if (!meta || meta.querySelector('.meta-groups')) return;
  const groups = {
    Score: ['time-select', 'key-select', 'add-measure-button', 'delete-measure-button'],
    Notation: ['lyric-button', 'chord-button', 'rehearsal-button', 'measure-tempo-button', 'repeat-start-button', 'repeat-end-button', 'navigation-button', 'volta-button', 'expression-button', 'text-style-button', 'cue-button', 'note-head-button', 'fingering-button', 'string-number-button', 'technique-button', 'arpeggio-button', 'trill-button', 'glissando-button', 'cross-staff-button', 'technique-text-button', 'ottava-kind', 'ottava-button', 'stem-button'],
    Parts: ['part-select', 'staff-select', 'part-view-button', 'add-part-button', 'delete-part-button', 'add-staff-button', 'delete-staff-button', 'rename-part-button', 'export-part-button', 'export-part-midi-button', 'part-group-button'],
    Layout: ['multi-rest-button', 'page-break-button', 'system-break-button'],
    Playback: ['pedal-button', 'midi-input-select', 'midi-input-button', 'midi-input-status']
  };
  const grouped = new Set(Object.values(groups).flat());
  const container = document.createElement('div'); container.className = 'meta-groups';
  Object.entries(groups).forEach(([label, ids]) => { const elements = ids.map((id) => $(id)).filter(Boolean); if (!elements.length) return; const details = document.createElement('details'); details.className = `meta-group meta-group-${label.toLowerCase()}`; if (label === 'Score') details.open = true; const summary = document.createElement('summary'); summary.textContent = label; details.append(summary, ...elements); container.append(details); });
  const extra = [...meta.children].filter((element) => !grouped.has(element.id) && !['project-title', 'project-summary', 'tempo-display', 'settings-button'].includes(element.id));
  if (extra.length) { const details = document.createElement('details'); details.className = 'meta-group meta-group-more'; const summary = document.createElement('summary'); summary.textContent = 'More'; details.append(summary, ...extra); container.append(details); }
...
function installUiSemantics() {
  document.querySelector('.editor-toolbar')?.setAttribute('role', 'toolbar');
  document.querySelector('.editor-toolbar')?.setAttribute('aria-label', 'Editing tools');
  document.querySelector('.transport')?.setAttribute('role', 'group');
  ...
  if (soundfont) soundfont.title = soundfont.textContent || 'SoundFont status';
