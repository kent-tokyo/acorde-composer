...
元ファイルを上書きせず、移行前後のファイルを残す運用を推奨します。Composer側はacorde v1.2.0のresolved preset-zone metadataを使い、実SF3 fixtureを非ゼロPCMへmaterializeする経路を検証しています。配布可能なSoundFont assetとproduction audio品質、完全な複数voice編集、MuseSounds接続、実OMR provider、コード署名済み配布は別の完了条件です。
...
- [MusicXML移行の確認手順](musicxml-migration-example.md)
- [ABC notationとMusicXMLの使い分け](abc-and-musicxml.md)
