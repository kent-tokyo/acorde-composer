...
現在は `acorde` v1.1.1の5 crateをpath依存かつversion pinして利用しています。必要な音楽機能が不足している場合は、Composer側で重複実装せず、先に `acorde` 側へ実装してテストします。
...
現在の検証結果はNode 169件、Rust unit test 20件です。notation coverageの12要素に個別MusicXML fixtureを割り当て、最新macOS arm64 packaged artifactで複数voiceのOpen → edit → save → reloadを確認しています。これは署名済み配布物やWindows実機QAの代替ではありません。詳細は[QA evidence](qa/README.md)、[CHANGELOG](CHANGELOG.md)、[GitHub Release v0.1.10](https://github.com/kent-tokyo/acorde-composer/releases/tag/v0.1.10)を参照してください。
