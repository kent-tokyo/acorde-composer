...
test('release QA report is deterministic and binds version and commit to all passing cases', () => {
  ...
  const report = createReleaseQaReport({ version: '0.1.5', commit: 'be680d5', matrix, results, releaseMetadataDigest: 'metadata-digest' });
  ...
  assert.equal(report.reportDigest, createReleaseQaReport({ version: '0.1.5', commit: 'be680d5', matrix, results, releaseMetadataDigest: 'metadata-digest' }).reportDigest);
});
...
test('release QA report rejects missing evidence and tampering', () => {
  ...
  const report = createReleaseQaReport({ version: '0.1.5', commit: 'be680d5', matrix, results: [] });
