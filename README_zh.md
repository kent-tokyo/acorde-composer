...
当前使用 `acorde` v1.1.7 的 5 个 crate，并通过 path dependency 和 version pin 固定。如果缺少音乐功能，不在 Composer 中复制实现，而是先在 `acorde` 中实现并测试。
...
当前已验证 Node 197 项、Rust unit test 20 项。`npm run check:candidate` 会按固定顺序运行 Node、静态检查、fixture、Playground、Rust test、clippy 和差分检查；需要机器可读的分阶段结果时可运行`npm run check:candidate:json`，需要包含依赖 provenance 的正式候选判定时可运行`npm run check:candidate:strict`。JSON 中`valid`表示本地验证，`releaseReady`表示正式候选是否就绪。notation coverage 的 12 个元素现在各自关联独立 MusicXML fixture，并加入了100／1,000／10,000小节性能fixture、cold start、playback、layout、SVG和repeat render测量CLI。最新 macOS arm64 packaged artifact 已确认多声部 Open → edit → save → reload 流程。这些结果不能替代已签名分发包或 Windows 真机 QA。详情请参阅 [QA evidence](qa/README.md)、[CHANGELOG](CHANGELOG.md) 和 [GitHub Release v0.1.13](https://github.com/kent-tokyo/acorde-composer/releases/tag/v0.1.13)。
