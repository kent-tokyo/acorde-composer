...
当前使用 `acorde` v1.0.9 的 5 个 crate，并通过 path dependency 和 version pin 固定。如果缺少音乐功能，不在 Composer 中复制实现，而是先在 `acorde` 中实现并测试。
...
当前已验证 Node 152 项、Rust unit test 18 项。这些结果不能替代已签名分发包或 macOS／Windows 真机 QA。详情请参阅 [QA evidence](qa/README.md)、[CHANGELOG](CHANGELOG.md) 和 [GitHub Release v0.1.8](https://github.com/kent-tokyo/acorde-composer/releases/tag/v0.1.8)。
